...
export class SwapMath {
  static MAX_SWAP_FEE = FEE_DENOMINATOR;
  ...
  static computeSwapStep(
    ...
    if (exactIn) {
      ...
      if (amountRemainingLessFee >= amountIn) {
        ...
        feeAmount =
          _feePips === SwapMath.MAX_SWAP_FEE
            ? amountIn
            : FullMath.mulDivRoundingUp(amountIn, _feePips, SwapMath.MAX_SWAP_FEE - _feePips);
      } else {
      ...
      feeAmount = FullMath.mulDivRoundingUp(amountIn, _feePips, SwapMath.MAX_SWAP_FEE - _feePips);
    }
