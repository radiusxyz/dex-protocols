...
exports.decodeEkuboCompactSqrtRatioToX128 = decodeEkuboCompactSqrtRatioToX128;
exports.compressTickFloor = compressTickFloor;
...
exports.buildTickBitmap = buildTickBitmap;
exports.setInitializedInBitmap = setInitializedInBitmap;
exports.clearInitializedInBitmap = clearInitializedInBitmap;
...
function decodeEkuboCompactSqrtRatioToX128(sqrtRatio) {
...
function compressTickFloor(tick, tickSpacing) {
    if (tickSpacing <= 0) {
...
function getSqrtRatioAtTick(tick) {
    if (!Number.isInteger(tick) || tick < constants_1.MIN_TICK || tick > constants_1.MAX_TICK) {
...
function getTickAtSqrtRatio(sqrtRatioX128) {
    if (!(sqrtRatioX128 >= constants_1.MIN_SQRT_RATIO && sqrtRatioX128 < constants_1.MAX_SQRT_RATIO)) {
        throw new Error('SQRT_RATIO');
    }
    let low = constants_1.MIN_TICK;
    let high = constants_1.MAX_TICK;
    while (low < high) {
...
function setBit(word, bitPos) {
    return word | (1n << BigInt(bitPos));
}
function clearBit(word, bitPos) {
    return word & ~(1n << BigInt(bitPos));
}
function setInitializedInBitmap(tickBitmap, tick, tickSpacing) {
    const compressed = compressTickFloor(tick, tickSpacing);
    const { wordPos, bitPos } = position(compressed);
    const prev = tickBitmap.get(wordPos) ?? 0n;
    tickBitmap.set(wordPos, setBit(prev, bitPos));
}
function clearInitializedInBitmap(tickBitmap, tick, tickSpacing) {
    const compressed = compressTickFloor(tick, tickSpacing);
    const { wordPos, bitPos } = position(compressed);
    const prev = tickBitmap.get(wordPos) ?? 0n;
...
function nextInitializedTickWithinOneWord(args) {
    ...
    if (lte) {
        const mask = ((1n << BigInt(bitPos + 1)) - 1n) & constants_1.MAX_UINT256;
    ...
    const mask = (constants_1.MAX_UINT256 ^ ((1n << BigInt(bitPos)) - 1n)) & constants_1.MAX_UINT256;
    const masked = bitmap & mask;
    if (masked !== 0n) {
