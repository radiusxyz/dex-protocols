...
exports.invertDecimalString = invertDecimalString;
exports.getInitializedTickIndicesFromWordFast = getInitializedTickIndicesFromWordFast;
...
function getAmount1Delta(sqrtRatioAX96, sqrtRatioBX96, liquidity, roundUp) {
    ...
    if (roundUp) {
        return (0, math_1.mulDivRoundingUp)(liquidity, delta, constants_1.Q96);
    }
    return (0, math_1.mulDiv)(liquidity, delta, constants_1.Q96);
}
/**
 ...
 */
function getNextSqrtPriceFromAmount0In(sqrtPX96, liquidity, amountIn) {
    if (amountIn === 0n) {
        return sqrtPX96;
    }
    ...
    return (0, math_1.mulDivRoundingUp)(numerator, sqrtPX96, denom);
}
/**
 ...
 */
function getNextSqrtPriceFromAmount1In(sqrtPX96, liquidity, amountIn) {
    if (amountIn === 0n) {
        return sqrtPX96;
    }
    const delta = (0, math_1.mulDiv)(amountIn, constants_1.Q96, liquidity);
...
function invertDecimalString(xStr, precision) {
    ...
    const xScaled = BigInt(i) * (0, math_1.pow10)(precision) + BigInt(frac === '' ? '0' : frac);
    if (xScaled === 0n) {
        return '0';
    }
    // (1 * 10^precision) / x  => scaled reciprocal
    const oneScaled = (0, math_1.pow10)(precision);
    const recipScaled = (oneScaled * (0, math_1.pow10)(precision)) / xScaled; // scale again to keep precision
    return formatFixed(recipScaled, precision);
}
// TODO: stompesi - check to remove
function getInitializedTickIndicesFromWordFast(wordPos, word, tickSpacing) {
    if (tickSpacing <= 0) {
        throw new Error('tickSpacing must be > 0');
    }
    if (word < 0n || word > constants_1.MAX_UINT256) {
    ...
    while (w !== 0n) {
        ...
        const bit = (0, tick_math_1.leastSignificantBit)(w); // 0..255
