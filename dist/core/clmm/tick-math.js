...
exports.nextInitializedTick = nextInitializedTick;
const constants_1 = require("./constants");
// helpers
function mulShift(val, mulByHex) {
...
function mostSignificantBit(x) {
    ...
    let msb = 0;
    for (const [power, min] of constants_1.POWERS_OF_2) {
        if (x >= min) {
            x >>= BigInt(power);
            msb += power;
        }
...
function getSqrtRatioAtTick(tick) {
    ...
    const quotient = ratio / constants_1.Q32;
    const remainder = ratio % constants_1.Q32;
    return remainder > 0n ? quotient + 1n : quotient;
}
function getTickAtSqrtRatio(sqrtRatioX96) {
    if (!(sqrtRatioX96 >= constants_1.MIN_SQRT_RATIO && sqrtRatioX96 < constants_1.MAX_SQRT_RATIO)) {
...
function nextInitializedTickWithinOneWord(args) {
    const UINT256_MASK = constants_1.MAX_UINT256;
...
function nextInitializedTick(args) {
