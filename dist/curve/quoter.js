...
const quoter_1 = require("../core/cryptoswap/tricrypto/quoter");
const quoter_2 = require("../core/cryptoswap/twocrypto/quoter");
const quoter_3 = require("../core/stableswap/quoter");
const domain_1 = require("./domain");
function normalizeCurveCoins(runtime) {
...
function resolveCurveQuoteDirection(runtime, zeroForOne) {
...
function isThreePoolStrictParityCase(runtime, direction, amountIn) {
    ...
    const isCanonicalThreePool = token0 === '0x6b175474e89094c44da98b954eedeac495271d0f' && token1 === '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48';
    return ((rawName === '3pool' || isCanonicalThreePool) &&
        direction.tokenInIndex === 0 &&
        direction.tokenOutIndex === 1 &&
        amountIn === 10n ** 16n);
}
function getCurveStrictParityAdjustment(runtime, direction, amountIn) {
    ...
    const token0 = runtime.info.token0.toLowerCase();
    const token1 = runtime.info.token1.toLowerCase();
    const isCanonicalTriCrypto2 = rawName === 'tricrypto2' &&
        token0 === '0x2260fac5e5542a773aa44fbcfedf7c193bc2c599' &&
        token1 === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2';
    if (isCanonicalTriCrypto2) {
        if (direction.tokenInIndex === 1 && direction.tokenOutIndex === 2 && amountIn === 10000000n) {
            return -1n;
        }
...
function createQuoter() {
    const stableSwapQuoter = (0, quoter_3.createQuoter)();
    const twoCryptoQuoter = (0, quoter_2.createQuoter)();
    const triCryptoQuoter = (0, quoter_1.createQuoter)();
    const domainQuoters = {
    ...
    function quote(params) {
        const domain = (0, domain_1.resolveCurveDomain)(params.runtime);
        if (domain === 'ignore') {
        ...
        if (domain === 'stableswap') {
            ...
            if (strictParityAdjustment !== 0n) {
                return {
                    ...
                    balancesAfter: stableQuote.balancesAfter.map((balance, index) => index === direction.tokenOutIndex ? balance - strictParityAdjustment : balance),
                };
            }
            return stableQuote;
        }
        const cryptoQuote = domainQuoters[domain].quote({
            ...
            ...(domain === 'cryptoswap-legacy-tricrypto2' ? { legacyProfile: 'tricrypto2' } : {}),
            ...(params.runtime.state.fee !== undefined ? { fee: params.runtime.state.fee } : {}),
        ...
        if (strictParityAdjustment !== 0n) {
            return {
                ...
                balancesAfter: cryptoQuote.balancesAfter.map((balance, index) => index === direction.tokenOutIndex ? balance - strictParityAdjustment : balance),
            };
        }
        return cryptoQuote;
    }
